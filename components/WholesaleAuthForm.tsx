    ...
    if (mode === 'signup') {
      ...
      const { data, error } = await supabase.auth.signUp({
        ...
        options: {
          data: {
            ...
            // NOTE: You can pass some non-sensitive metadata here
            // but sensitive info should be added to the 'profiles' table post-signup.
      ...
      if (error) {
        setMessage({ type: 'error', text: error.message });
         ...
         // TODO: In a real-world app, you would now:
         // 1. Upload the `profilePicture` and `idDocument` files to Supabase Storage.
         //    e.g., await supabase.storage.from('avatars').upload(`public/${data.user.id}`, profilePicture);
         // 2. Save the rest of the form data (shippingAddress, phone, etc.) and the file URLs 
         //    to the `profiles` table, which would need to be extended with these columns.
  ...
  const renderSignupForm = () => (
    <form className="space-y-6" onSubmit={handleAuth}>
        ...
        <fieldset className="space-y-4">
         ...
         <fieldset className="space-y-4">
            <legend className="text-lg font-semibold border-b w-full pb-2 mb-2">Wholesaler Identity Information</legend>
            <div><label htmlFor="companyName" className={formLabelClasses}>Company / Business Name</label><input id="companyName" type="text" required value={companyName} onChange={e => setCompanyName(e.target.value)} className={formInputClasses}/></div>
            <div>
